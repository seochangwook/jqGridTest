...
public interface UserInfoDao extends JpaRepository<UserInfo, String>{
	...
	@Query(nativeQuery = true, value="select c.user_id from userinfo c where c.user_email = :user_email AND c.user_id = :user_id")
	List<UserInfo> findByUserPhoneNumber(@Param("user_email") String user_email, @Param("user_id") String user_id);
	
	@Transactional
	@Modifying
	@Query(nativeQuery = true, value="delete from userinfo c where c.user_id = :user_id")
